...
let currentWatcher: NodeJS.Timeout | null = null;
...
export function toggleBigOpenFolderButton(): void {
    ...
    try {
        ...
        if (err instanceof Error) {
            ...
            if (
                ...
                if (statusMsg)
                    statusMsg.textContent =
                        'Network error: Cannot access directory';
            } else {
                if (statusMsg)
...
export async function createNewFile(): Promise<void> {
    ...
    try {
        ...
        if (statusMsg)
            statusMsg.textContent = `Created file: ${trimmedFileName}`;
    } catch (error) {
        if (error instanceof Error) {
            ...
            if (
                ...
                if (statusMsg)
                    statusMsg.textContent =
                        'Access forbidden: This path is restricted';
            } else {
                if (statusMsg)
...
function watchFolderChanges(folderPath: string): void {
    ...
    const intervalId = setInterval(async () => {
        ...
        try {
            ...
            /* Error checking folder changes */
        }
    }, 2000);

    currentWatcher = intervalId;
}
